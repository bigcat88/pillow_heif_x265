...
class RequiredDependencyException(Exception):
    """Raised when ``x265`` or ``libheif`` is not found."""
...
class PillowHeifBuildExt(build_ext):
    ...
    def build_extensions(self):  # noqa
        ...
        extra_compile_args = ["-std=c++11", "-DPLUGIN_X265"]
        ...
        x265_include = self._find_include_dir("libx265", "x265.h")
        if not x265_include:
            raise RequiredDependencyException("x265")
